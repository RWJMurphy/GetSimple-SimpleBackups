...
<form id="add" action="<?php echo sb_link("add_schedule"); ?>" method="POST">
    <p><label for="name"><?php i18n(SB_SHORTNAME.'/NAME'); ?>:</label><input class="text" name="name" value="<?php if (isset($_SESSION['add_schedule']['name'])) { echo $_SESSION['add_schedule']['name']; } ?>" /></p>
...
<option <?php if (isset($_SESSION['add_schedule']['frequency']) && $_SESSION['add_schedule']['frequency'] == $frequency) { echo 'selected="selected" '; } ?> value="<?php echo $frequency; ?>"><?php echo $name; ?></option>
<?php } ?>
    </select></p>
    <p><label for="limit"><?php i18n(SB_SHORTNAME.'/BACKUP_LIMIT'); ?>:</label><input class="text" name="limit" value="<?php if (isset($_SESSION['add_schedule']['limit'])) { echo $_SESSION['add_schedule']['limit']; } ?>" /></p>
