...
<form id="add" action="<?php echo sb_link("add_source"); ?>" method="POST">
    ...
    <p><label for="name"><?php i18n(SB_SHORTNAME.'/NAME'); ?>:</label><input class="text" name="name" value="<?php if (isset($_SESSION['add_source']['name'])) { echo $_SESSION['add_source']['name']; } ?>" /></p>
<fieldset class="type" id="fs_local"></p>
    <p><label form="local_path"><?php i18n(SB_SHORTNAME.'/PATH'); ?>:</label><input class="text" name="local_path" value="<?php if (isset($_SESSION['add_source']['local_path'])) { echo $_SESSION['add_source']['local_path']; } else { echo GSBACKUPSPATH . "simplebackups/"; } ?>" /></p>
</fieldset>
